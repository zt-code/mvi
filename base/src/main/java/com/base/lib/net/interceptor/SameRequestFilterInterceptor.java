...
public class SameRequestFilterInterceptor implements Interceptor {
        ...
        if (response.isSuccessful() && response.body() != null) {
            ...
            getMainHandler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    L.i(config.responseCacheTimeInMills() + "时间到了,清除缓存的response");
                }
    ...
    private boolean needwait(String key) {
        if (responseWeakHashMap.containsKey(key)) {
            L.i("有缓存的response,直接去读缓存,并组装新的response");
            return false;
        }
        if (calls.containsKey(key)) {
            WeakReference<Call> callWeakReference = calls.get(key);
            if (callWeakReference == null) {
                L.i("不需要等待,直接发请求 call WeakReference not exist:");
                return false;
            }
            Call call = callWeakReference.get();
            if (call == null || call.isCanceled()) {
                L.i("不需要等待,直接发请求 call not exist or is canceld:" + call);
                return false;
            }
            L.i("请求可能正在等待或正在执行-needwait call is running:" + call);
            //请求可能正在等待或正在执行
            return true;
        }
        L.i("任何地方都没有,不需要等,直接执行请求");
        //任何地方都没有,不需要等,直接执行请求
        return false;
    }
